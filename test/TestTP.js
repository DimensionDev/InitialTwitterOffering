...
contract("HappyTokenPool", accounts => {
    ...
    describe("claim()", async () => {
        ...
        it("Should claim various numbers of token", async () => {
            ...
            await pool.claim.sendTransaction(pool_id, vr.verification, accounts[5], vr.validation, 0, exchange_amount, {'from': accounts[5], 'value': approve_amount})
            const logs_eth = await web3.eth.getPastLogs({address: pool.address, topics: [web3.utils.sha3(claim_success_encode)]})
            ...
            await pool.claim.sendTransaction(pool_id, vr.verification, accounts[4], vr.validation, 1, exchange_amount, {'from': accounts[4]})
            const logs_b = await web3.eth.getPastLogs({address: pool.address, topics: [web3.utils.sha3(claim_success_encode)]})
            ...
            await pool.claim.sendTransaction(pool_id, verification, account, validation, tokenC_address_index, exchange_amount, {'from': account})
            const logs_c = await web3.eth.getPastLogs({address: pool.address, topics: [web3.utils.sha3(claim_success_encode)]})
