...
describe("HappyTokenPool", () => {
  ...
  describe("swap()", async () => {
    ...
    it("Should throw error when password wrong", async () => {
      const { id: pool_id } = await getResultFromPoolFill(happyTokenPoolDeployed, fpp);
      await expect(
        happyTokenPoolDeployed.swap(pool_id, "wrong password", tokenC_address_index, exchange_amount, [pool_id]),
      ).to.be.rejectedWith(Error);
    });
    ...
    it("Should swap various numbers of token", async () => {
      ...
      await happyTokenPoolDeployed.connect(signers[3]).swap(pool_id, vr.verification, 1, exchange_amount, [pool_id]);
    ...
    describe("claim()", async () => {
      ...
      it("should emit multiple ClaimSuccess events when claim successfully and set claimable to zero", async () => {
        ...
        const availabilityPrevious2 = await getAvailability(happyTokenPoolDeployed, pool_id2, pool_user.address);
        ...
        expect(contractTokenABalanceAfterClaim.toString()).to.be.eq(
          BigNumber(contractTokenABalanceBeforeClaim.toString()).minus(BigNumber(exchangedTokenA_total)).toFixed(),
        );
    ...
    describe("setUnlockTime()", async () => {
      it("should setUnlockTime work", async () => {
        ...
        {
          // can NOT set to 0
          await expect(happyTokenPoolDeployed.connect(creator).setUnlockTime(pool_id, 0)).to.be.rejectedWith(Error);
        }
        if (true) {
