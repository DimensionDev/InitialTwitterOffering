...
describe("HappyTokenPool", () => {
    ...
    describe("fill_pool()", async () => {
        ...
        it("Should emit fillSuccess event correctly when a happyTokenPoolDeployed is filled", async () => {
            ...
            const logs = await ethers.provider.getLogs({address: happyTokenPoolDeployed.address, topics: [ethers.utils.keccak256(ethers.utils.toUtf8Bytes(fill_success_encode))]})
    ...
    describe("check_availability()", async () => {
        ...
        it("Should throw error when poor id does not exist", async () => {
            await expect(happyTokenPoolDeployed.check_availability('id not exist', {'from': signers[1].address})).to.be.rejectedWith(Error)
        })
        ...
        it("Should return the exchanged_tokens filled with zero when there was no exchange", async () => {
            const { id: pool_id } = await getResultFromPoolFill(happyTokenPoolDeployed, fpp)
            const result = await getAvailability(happyTokenPoolDeployed, pool_id, signers[1].address)
            expect(result.exchanged_tokens.map(bn => ethers.utils.parseEther(bn.toString()).toString())).to.eql(['0', '0', '0'])
        })
        ...
        it("Should minus the number of remaining token by exchange amount after swap", async () => {
            ...
            happyTokenPoolDeployed.connect(signer).swap(pool_id, verification, signer.address, validation, tokenB_address_index, approve_amount)
    ...
    describe("swap()", async () => {
        ...
        it("Should throw error when happyTokenPoolDeployed id does not exist", async () => {
            ...
            await expect(
                happyTokenPoolDeployed.swap(pool_id, verification, signers[2].address, validation, tokenC_address_index, exchange_amount)
        ...
        it("Should throw error when validation wrong", async () => {
            const { id: pool_id } = await getResultFromPoolFill(happyTokenPoolDeployed, fpp)
            await expect(
                happyTokenPoolDeployed.swap(pool_id, verification, signers[2].address, "wrong validation", tokenC_address_index, exchange_amount)
            ).to.be.rejectedWith(Error)
        })
        ...
        it("Should throw error when happyTokenPoolDeployed is expired", async () => {
            ...
            const { id: pool_id } = await getResultFromPoolFill(happyTokenPoolDeployed, fpp)
            expect(
                happyTokenPoolDeployed.connect(signers[2]).swap(pool_id, verification, signers[2].address, validation, tokenC_address_index, exchange_amount)
            ).to.be.rejectedWith(Error)
        })
        ...
        it("Should throw error when password wrong", async () => {
            const { id: pool_id } = await getResultFromPoolFill(happyTokenPoolDeployed, fpp)
            await expect(
                happyTokenPoolDeployed.swap(pool_id, "wrong password", signers[2].address, validation, tokenC_address_index, exchange_amount)
            ).to.be.rejectedWith(Error)
        })
        ...
        it("Should throw error when one account swap more than once", async () => {
            ...
            await happyTokenPoolDeployed.connect(signers[2]).swap(pool_id, verification, signers[2].address, validation, tokenC_address_index, exchange_amount)
            expect(
                happyTokenPoolDeployed.connect(signers[2]).swap(pool_id, verification, signers[2].address, validation, tokenC_address_index, exchange_amount)
            ).to.be.rejectedWith(Error)
        })
        ...
        it("Should emit swapSuccess when swap successful", async () => {
            ...
            await happyTokenPoolDeployed.connect(signers[2]).swap(pool_id, verification, signers[2].address, validation, tokenC_address_index, exchange_amount)
            const logs = await ethers.provider.getLogs({address: happyTokenPoolDeployed.address, topics: [ethers.utils.keccak256(ethers.utils.toUtf8Bytes(swap_success_encode))]})
        ...
        it("Should swap the maximum number of token equals to limit", async () => {
            ...
            expect(result.to_value.toString()).to.be.eq(fpp.limit).and.to.not.be.eq(String(exchange_amount * ratio))
        })
        ...
        it("Should swap various numbers of token", async () => {
            ...
            await happyTokenPoolDeployed.connect(signers[4]).swap(pool_id, vr.verification, signers[4].address, vr.validation, 0, exchange_amount, {'value': approve_amount})
            const logs_eth = await ethers.provider.getLogs({address: happyTokenPoolDeployed.address, topics: [ethers.utils.keccak256(ethers.utils.toUtf8Bytes(swap_success_encode))]})
            ...
            await happyTokenPoolDeployed.connect(signers[3]).swap(pool_id, vr.verification, signers[3].address, vr.validation, 1, exchange_amount)
            const logs_b = await ethers.provider.getLogs({address: happyTokenPoolDeployed.address, topics: [ethers.utils.keccak256(ethers.utils.toUtf8Bytes(swap_success_encode))]})
            ...
            await happyTokenPoolDeployed.connect(signers[2]).swap(pool_id, verification, signers[2].address, validation, tokenC_address_index, exchange_amount)
            const logs_c = await ethers.provider.getLogs({address: happyTokenPoolDeployed.address, topics: [ethers.utils.keccak256(ethers.utils.toUtf8Bytes(swap_success_encode))]})
        ...
        it('Should swap the remaining token when the amount of swap token is greater than total token', async () => {
            ...
            await happyTokenPoolDeployed.connect(signers[2]).swap(
                pool_id,
                v1.verification,
                swapperFirstETH,
                v1.validation,
            ...
            await happyTokenPoolDeployed.connect(signers[3]).swap(
                pool_id,
                v2.verification,
                signers[3].address,
                v2.validation,
    ...
    describe("destruct()", async () => {
        ...
        it("Should emit DestructSuccess event and withdraw all tokens", async () => {
            ...
            const logs = await ethers.provider.getLogs({address: happyTokenPoolDeployed.address, topics: [ethers.utils.keccak256(ethers.utils.toUtf8Bytes(destruct_success_encode))]})
            ...
            expect(BigNumber(tokenB_balance.toString()).toString()).to.be.eq(
                BigNumber(previous_tokenB_balance.toString()).minus(transfer_amount).plus(BigNumber(exchange_tokenB_amount)).toString()
            ).and.to.be.eq(
                BigNumber("9e26").plus(BigNumber("2e10")).toString()
            )

            const tokenC_balance = await testTokenCDeployed.balanceOf(creator.address)
            expect(BigNumber(tokenC_balance.toString()).toString()).to.be.eq(
                BigNumber(previous_tokenC_balance.toString()).minus(transfer_amount).plus(BigNumber(exchange_tokenC_amount)).toString()
            ).and.to.be.eq(
                BigNumber("80001e22").toString()
            )
        })
        ...
        it("Should emit DestructSuccess event and withdraw all tokens", async () => {
            ...
            expect(tokenB_balance).to.be.eq(
                BigNumber(previous_tokenB_balance.toString()).minus(BigNumber(transfer_amount)).plus(BigNumber(exchange_tokenB_amount)).toFixed()
            )

            const tokenC_balance = await testTokenCDeployed.balanceOf(creator.address)
            expect(tokenC_balance).to.be.not.eq(
                BigNumber(previous_tokenC_balance.toString()).minus(BigNumber(transfer_amount)).plus(
    ...
    async function approveThenSwapToken (test_token, swapper, token_address_index, pool_id, exchange_amount) {
        ...
        await happyTokenPoolDeployed.connect(swapper).swap(pool_id, verification, swapper.address, validation, token_address_index, exchange_amount)
    }
