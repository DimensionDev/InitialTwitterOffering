...
contract HappyTokenPool {
    ...
    **/
    function fill_pool (bytes32 _hash, uint256 _start, uint256 _end, string memory name, string memory message,
                        address[] memory _exchange_addrs, uint128[] memory _prices,
    ...
    public payable {
        ...
        require(IERC721(_token_addr).isApprovalForAll(msg.sender, address(this)) == true, "Not approved for all");
        require(IERC721(_token_addr).balanceOf(msg.sender) == _total_tokens, "Insuffcient allowance");
        require(_exchange_addrs.length > 0, "Exchange token addresses need to be set");
        require(_prices.length == _exchange_addrs.length, "Size of ratios = size of exchange_addrs");
    ...
    **/
    ...
    public payable returns (uint256 swapped) {
        ...
        require (verification == keccak256(abi.encodePacked(unbox(pool.packed1, 160, 40), msg.sender)), 
                 'Wrong Password');
        ...
        require (pool.swapped_map[msg.sender] <= , "Already swapped");
    ...
    **/
    ...
    function wrap1 (address _token_addr, bytes32 _hash, uint256 _start, uint256 _end) internal pure 
        ...
        _packed1 |= box(160, 40, uint256(_hash) >> 208);    // hash = 40 bits (safe?)
