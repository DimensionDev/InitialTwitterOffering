...
contract HappyTokenPool is Initializable {
    ...
    event WithdrawSuccess(bytes32 indexed id, address indexed token_address, uint256 withdraw_balance);
    ...
    function swap(
        ...
        require(verification == keccak256(abi.encodePacked(uint256(packed1.password), msg.sender)), "Wrong Password");
        ...
        if (swapped_tokens > packed2.limit) {
            ...
            // return the eth
            if (exchange_addr == DEFAULT_ADDRESS) payable(msg.sender).transfer(msg.value - from_value);
        }
        // make sure again
        require(swapped_tokens <= packed2.limit);
        // update exchanged
        pool.exchanged_tokens[exchange_addr_i] = SafeCast.toUint128(
            SafeMath.add(pool.exchanged_tokens[exchange_addr_i], from_value)
        );
        ...
        )
    {
        ...
        bool _claimed = false;
        if (pool.swap_status[msg.sender].claimed || ((_swapped_amount != 0) && (packed3.unlock_time == 0))) {
            _claimed = true;
        }
        return (
    ...
    function claim(bytes32[] calldata ito_ids) external {
        for (uint256 i = 0; i < ito_ids.length; i++) {
            ...
            Packed3 memory packed3 = pool.packed3;
            if (packed3.unlock_time == 0) continue;
            if (packed3.unlock_time + base_time > block.timestamp) continue;
            if (pool.swap_status[msg.sender].claimed) continue;
            uint256 claimed_amount = pool.swapped_map[msg.sender];
            if (claimed_amount == 0) continue;
            pool.swap_status[msg.sender].claimed = true;
        ...
        for (uint256 i = 0; i < pool.exchange_addrs.length; i++) {
            if (pool.exchanged_tokens[i] > 0) {
                ...
                if (pool.exchange_addrs[i] != DEFAULT_ADDRESS)
                    ...
                    // ETH
                else payable(msg.sender).transfer(pool.exchanged_tokens[i]);
            }
        }
