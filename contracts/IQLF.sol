...
abstract contract IQLF is IERC165 {
     ...
     */
    function ifQualified(address account, bytes32[] memory data) external view virtual returns (bool, string memory);

    /**
     * @dev Logs if the given address is qualified, implemented on demand.
     */
    function logQualified(address account, bytes32[] memory data) external virtual returns (bool, string memory);
